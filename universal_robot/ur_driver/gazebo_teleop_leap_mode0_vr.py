...
global palm_centre_pre, palm_centre, grab_strength, g, roll_pre, pitch_pre, yaw_pre, teleoperation_mode
...
def Leap_callback_xyz(status):
    ...
    if(teleoperation_mode == "MODE_0"):
        ...
        if (grab_strength > 0.9 and palm_centre_pre is not None): 
            ...
            print("Del(X,Y,Z) = ("+ str(del_x) + ", " + str(del_y) + ", " + str(del_z) + ")\n")
            ...
            pose_xyz = np.add(pose_xyz, del_xyz)
            pose = kin.inverse(pose_xyz,q_guess) # Reconvert the desired xyz onto the joint space  
            ...
            print "MODE0: Position your left hand on the sensor and close your hand"
...
def listener_from_leap():
    ...
    rospy.Subscriber("/leap_motion/leap_filtered",LeapMsg, Leap_callback_xyz, queue_size=1)
...
def main():
    ...
    try:
        rospy.init_node("endeffector_controller_by_leap_Mode0", disable_signals=True)        
