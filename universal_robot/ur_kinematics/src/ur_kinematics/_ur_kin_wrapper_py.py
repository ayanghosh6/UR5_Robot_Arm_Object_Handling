...
from ur_kinematics._ur_kin_wrapper_cpp import forward as ur5_forward
from ur_kinematics._ur_kin_wrapper_cpp import inverse as ur5_inverse
    ...
    def inverse_all(self, x, q_guess=None, q_min=6*[-2.*np.pi], q_max=6*[2.*np.pi]):
        ...
        for sol in sols:
            ...
            if np.all(test_sol != 9999.):
                ...
                if np.allclose(np.linalg.inv(self.forward(test_sol)) * x, np.eye(4), atol=1e-06):
